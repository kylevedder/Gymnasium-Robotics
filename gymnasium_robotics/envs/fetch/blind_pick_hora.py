...
class FetchBlindPickEnv(MujocoFetchEnv, EzPickle):
    ...
    def __init__(self, camera_names=None, reward_type="dense", obj_range=0.07, include_obj_state=False, **kwargs):
        ...
        _obs_space = {}
        self.history_len = 30
        # even though observation is 12 dim, hora code assumes obs is 32 dim. 
        self.history_buffer = np.zeros((self.history_len, 32), dtype=np.float32)
        ...
        _obs_space["obs"] = spaces.Box(-np.inf, np.inf, shape=(12,), dtype="float32")
        self.include_obj_state = include_obj_state
        if include_obj_state:
            _obs_space["priv_info"] = spaces.Box(-np.inf, np.inf, shape=(3,), dtype="float32")
    ...
    def _get_obs(self):
        ...
        if hasattr(self, "mujoco_renderer"):
            ...
            obs["obs"] = np.concatenate([grip_pos, grip_velp, gripper_state, gripper_vel, touch]).astype(np.float32)
            if self.include_obj_state:
                obj0_pos = self._utils.get_site_xpos(self.model, self.data, "object0").copy()
                obs["priv_info"] = obj0_pos.astype(np.float32)
    ...
    def step(self, action):
        ...
        padded_obs = np.pad(obs["obs"], ((0, 32 - len(obs["obs"]))), constant_values=0)
        self.history_buffer[0] = padded_obs
