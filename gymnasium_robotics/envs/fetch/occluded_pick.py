...
MODEL_XML_PATH = os.path.join("fetch", "occluded_pick.xml")
...
class FetchOccludedPickEnv(MujocoFetchEnv, EzPickle):
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            ...
            # update indicator site position so that it is always above the object
            indicator_pos = [1.47, 0.75, 0.5]
            indicator_pos[1] = object_xpos[1]
            # add some noise to the y position
            indicator_pos[1] += self.np_random.uniform(-0.05, 0.05)
            self.indicator_pos = indicator_pos
            body_id = self._mujoco.mj_name2id(
                self.model, self._mujoco.mjtObj.mjOBJ_BODY, "indicator"
            )
            self.model.body_pos[body_id] = self.indicator_pos
...
if __name__ == "__main__":
    env = FetchOccludedPickEnv(["behind_camera"], "dense", render_mode="human", width=64, height=64)
    # # returns = 0
    # while True:
    env.reset()
